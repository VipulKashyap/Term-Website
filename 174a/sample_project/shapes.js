function otto( points_transform )
{
    shape.call(this);
...
inherit(otto, shape);
otto.prototype.calcRadius = function (y) {
	var ySquared = (y * y);
	if(ySquared > 1)
		return 0;
	return 0.5 * Math.sqrt(1 - ySquared) + 0.3 * (1 - y);
};

// the rectangle looks smoother
// otto.prototype.populate = function ( recipient, points_transform )
// {
// 	var m_rect = new triangle_strip();
// 	var indices = [0, 1, 2];
...
// 	var deltaY = 0.02;
// 	var deltaDeg = 5;
// 	for(var y = -1; y < 1; y += deltaY)
// 	{
// 		var radius = this.calcRadius(y);
// 		var belowY = y + deltaY;
// 		var belowRadius = this.calcRadius(belowY);
//
// 		for(var deg = 0; deg < 360; deg += deltaDeg)
// 		{
// 			var x = radius * Math.cos (deg);
// 			var z = radius * Math.sin (deg);
// 			var nextX = radius * Math.cos (deg + deltaDeg);
// 			var nextZ = radius * Math.sin (deg + deltaDeg);
// 			var belowX = belowRadius * Math.cos (deg);
// 			var belowZ = belowRadius * Math.sin (deg);
// 			var nextBelowX = belowRadius * Math.cos (deg + deltaDeg);
// 			var nextBelowZ = belowRadius * Math.sin (deg + deltaDeg);
//
// 			vertices.push( vec3(x, y, z) );
// 			vertices.push( vec3(nextX, y, nextZ) );
// 			vertices.push( vec3(nextBelowX, belowY, nextBelowZ) );
//
// 			vertices.push( vec3(x, y, z) );
// 			vertices.push( vec3(belowX, belowY, belowZ) );
// 			vertices.push( vec3(nextBelowX, belowY, nextBelowZ) );
// 		}
// 	}
//
// 	var maxI = vertices.length;
// 	for (var i = 0; i < maxI; i+=3) {
// 		m_rect.init_from_strip_lists( recipient, [vertices[i], vertices[i+1], vertices[i+2]], indices );
// 	}
//
...
function generateIndicies(vecArray)
{
  var indices = [];
  var indCounter = 0;
  var indiciesHashTable = {};

  for(var i = 0; i < vecArray.length; i++)
  {
    var vecKey = getVecKey(vecArray[i]);
    if(!indiciesHashTable[vecKey])
    {
      indiciesHashTable[vecKey] = indCounter++;
    }
    indices.push(indiciesHashTable[vecKey]);
  }
...
otto.prototype.populate = function ( recipient, points_transform )
{
	var m_rect = new rectangular_strip();
	var index_offset = recipient.indices.length;
	var vertices = [];
  ...
  var xTex = 0;
  var yTex = 0;
  var deltXTex = deltaDeg/360;
  var deltYTex = deltaY/2;

	var maxI = vertices.length;
	for (var i = 0; i < maxI; i+=4) {
		m_rect.init_from_strip_lists( recipient, [vertices[i], vertices[i+1], vertices[i+2], vertices[i+3]], [0, 1, 2, 3] );
    recipient.texture_coords[i] = vec2(xTex, yTex);
    recipient.texture_coords[i + 1] = vec2(xTex, yTex + deltYTex);
    recipient.texture_coords[i + 2] = vec2(xTex + deltXTex, yTex);
    recipient.texture_coords[i + 3] = vec2(xTex + deltXTex, yTex + deltYTex);
    if(xTex == 1){
      xTex = 0;
      yTex += deltYTex;
    }else{
...
inherit(star, shape);
star.prototype.populate = function ( recipient )
{
	var m_tri = new triangle_strip();
	var index_offset = recipient.indices.length;
	var vertices = [];

	for(var onLeft = -1; onLeft < 2; onLeft+=2)
	{
		for(var onBack = -1; onBack < 2; onBack+=2)
		{
			// Side 1
			vertices.push( vec3(0, 0, 0.12 * onBack) );
			vertices.push( vec3(0, -0.27365906962, 0) );
			vertices.push( vec3(0.29389262614 * onLeft, -0.40450849719, 0) );

			// Side 2
			vertices.push( vec3(0, 0, 0.12 * onBack) );
			vertices.push( vec3(0.2602652414 * onLeft, -0.08456530317, 0) );
			vertices.push( vec3(0.29389262614 * onLeft, -0.40450849719, 0) );

			// Side 3
			vertices.push( vec3(0, 0, 0.12 * onBack) );
			vertices.push( vec3(0.2602652414 * onLeft, -0.08456530317, 0) );
			vertices.push( vec3(0.475528258 * onLeft, 0.154508497, 0) );

			// Side 4
			vertices.push( vec3(0, 0, 0.12 * onBack) );
			vertices.push( vec3(0.475528258 * onLeft, 0.154508497, 0) );
			vertices.push( vec3(0.16085276528 * onLeft, 0.22139483799, 0) );
	...
	for (var i = 0; i < 60; i+=3) {
		m_tri.init_from_strip_lists( recipient, [vertices[i], vertices[i+1], vertices[i+2]], [0, 1, 2] );
    this.texture_coords.push( vec2(0, 0), vec2(1, 0), vec2(0, 1) );
	}
...
pyramid.prototype.populate = function ( recipient )
{
  var m_tri = new triangle_strip();
  var m_rect = new rectangular_strip();
  var indices = [0, 1, 2];
  ...
  vertices.push( vec3(0.5, 0.0, 0.5) );
  texture_coords.push( vec2(1, 0) );
  vertices.push( vec3(-0.5, 0.0, 0.5) );
  texture_coords.push( vec2(0, 0) );
  vertices.push( vec3(0.0, 1, 0.0) );
  texture_coords.push( vec2(0.5, 0.5) );

  vertices.push( vec3(-0.5, 0.0, -0.5) );
  texture_coords.push( vec2(0, 1) );
  vertices.push( vec3(-0.5, 0.0, 0.5) );
  texture_coords.push( vec2(1, 1) );
  vertices.push( vec3(0.0, 1, 0.0) );
  texture_coords.push( vec2(0.5, 0.5) );
  ...
  for (var i = 0; i < 12; i+=3) {
	   m_tri.init_from_strip_lists( recipient, [vertices[i], vertices[i+1], vertices[i+2]], indices );
     recipient.texture_coords[i] = texture_coords[i];
     recipient.texture_coords[i + 1] = texture_coords[i + 1];
     recipient.texture_coords[i + 2] = texture_coords[i + 2];
  }
  ...
  m_rect.populate( recipient, 1, mult( mat4(), rotation(90,0,0,1) ));	// Make 1 square for bottom
...
function sphere(points_transform, max_subdivisions) // Build a complicated sphere using subdivision, starting with a simple tetrahedron.
{
    ...
    this.subdivideTriangle = function(a, b, c, recipient, count) // This function will recurse through each level of detail by splitting triangle (a,b,c) into four smaller triangles.
        {
            ...
            var ab = recipient.vertices.length;
            recipient.vertices.push(ab_vert); // The indices of the three new vertices
            var ac = recipient.vertices.length;
            recipient.vertices.push(ac_vert);
            var bc = recipient.vertices.length;
            recipient.vertices.push(bc_vert);
    ...
    this.draw = function(graphicsState, model_transform, material, LOD) {
        ...
        if (material.texture_filename && textures[material.texture_filename].loaded) // Omit the texture string parameter to signal that we don't want to texture this shape.
        {
            ...
            g_addrs.shader_attributes[2].enabled = false;
        }
...
function text_line(string_size) // Draws a rectangle textured with images of ASCII characters over each quad, spelling out a string.
{
    ...
    this.draw = function(graphicsState, model_transform, heads_up_display, color) {
        if (heads_up_display) {
            ...
            var temp_camera_transform = graphicsState.camera_transform;
            graphicsState.camera_transform = mat4();
        }
        shape.prototype.draw.call(this, graphicsState, model_transform, new Material(color, 1, 0, 0, 40, "text.png"));
        if (heads_up_display) {
            gl.enable(gl.DEPTH_TEST);
            graphicsState.camera_transform = temp_camera_transform;
        }
    }
