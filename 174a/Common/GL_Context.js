...
{	this.string_map = { };	this.m_text = new text_line( 20 ); 		this.start_index = 0;	this.tick = 0; 	this.visible = true;
	...
	Debug_Screen.prototype.display = function(time)
	{
		...
		model_transform     = mult( model_transform, translation( .1, -.9, 0.95 ) );
		model_transform     = mult( model_transform, scale( 1, .075, .05) );

		var strings = Object.keys( this.string_map );
		var stack = [];
		
		for( var i = 0, idx = this.start_index; i < 5 && i < strings.length; i++, idx = (idx + 1) % strings.length )
		{
			stack.push(model_transform);
			if(idx == 4)
			{
				stack.push(model_transform);
				model_transform = mult( model_transform, translation( 0, 9, -2 ) );
				model_transform = mult( model_transform, scale( 4, 4, 4 ) );
			}
			if(idx == 3)
			{
				stack.push(model_transform);
				model_transform = mult( model_transform, translation( 0, 21, 0 ) );
				model_transform = mult( model_transform, scale( 1.3, 1.3, 1.3 ) );
			}
			if(idx == 2)
			{
				model_transform = mult( model_transform, translation( 0, 20.5, 0 ) );
				model_transform = mult( model_transform, scale( 1.1, 1.1, 1.1 ) );
			}
			if(idx == 1)
			{
				model_transform = mult( model_transform, translation( 0, -1, -33 ) );
			}
			if(idx == 0)
			{
