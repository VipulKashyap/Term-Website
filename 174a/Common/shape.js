	...
	shape.prototype.update_uniforms = function( graphicsState, model_transform, material )			// Send the current matrices to the shader
		{
				...
				var N_LIGHTS = 3, lightPositions = [], lightColors = [], attenuations = [],
                lightPositions_flattened = [], lightColors_flattened = [];
				lightPositions.push( vec4( 10 * Math.sin(4800/1000), 2, -2, 1 ) );
				lightColors.push( vec4( 1, 1, 1, 1 ) );
				attenuations.push( .0001 );

				lightPositions.push( vec4( 2, 10 * Math.sin(4800/1000), -2, 1 ) );
				lightColors.push( vec4( 1, 0, 0, 1 ) );
				attenuations.push( .0001 );

				lightPositions.push( vec4( 2, -2, 10 * Math.sin(4800/1000), 1 ) );
